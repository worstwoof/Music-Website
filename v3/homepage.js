    ...
    for (let i = 0; i < count; i++) {
        html += `
            <div class="skeleton-card">
                ...
                <div class="skeleton-bg skeleton-content">
...
async function renderHomepageSections() {
    ...
    // 2. 【关键】在请求数据前，先立即展示骨架屏
    const skeletonHTML = getSkeletonHTML(5); // 生成 5 个骨架卡片
    if (discoverTrack) discoverTrack.innerHTML = skeletonHTML;
    if (newReleaseTrack) newReleaseTrack.innerHTML = skeletonHTML;

    // 模拟网络延迟 (可选：为了让你看清骨架屏效果，正式上线可去掉下面这行 setTimeout)
    ...
    if (allSongs.length === 0) {
        console.warn("未能加载任何歌曲信息。");
        // 加载失败时，清空骨架或显示错误提示
        if (discoverTrack) discoverTrack.innerHTML = '<p style="padding:20px">暂无推荐</p>';
        if (newReleaseTrack) newReleaseTrack.innerHTML = '';
        return;
    }
    ...
    // 5. 【重要】因为内容改变了，需要重新初始化 Marquee 动画和交互特效
    // 如果之前已经在 DOMContentLoaded 里调用了这些，这里需要重新调用或刷新
    // (根据你目前的结构，DOM加载完就会执行下面的初始化，所以这里只要确保
    //  renderHomepageSections 是最先执行的即可，不需要额外操作)
    
    console.log("动态内容渲染完毕 (骨架屏已移除)。");
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  initTiltEffects();       // 3D 倾斜
  initGsapHoverEffects();  // GSAP 悬停缩放
...
// =======================================
  ...
  (async function initSearchData() {
      // 复用已有的 fetchAllSongInfo 函数
      searchSongsCache = await fetchAllSongInfo();
      console.log(`[主页搜索] 数据已就绪: ${searchSongsCache.length} 首歌曲`);
  })();
  ...
  if (searchInput && searchDropdown) {
      ...
      searchInput.addEventListener("input", (e) => {
          ...
          const matches = searchSongsCache.filter(song => {
              return (song.title && song.title.toLowerCase().includes(keyword)) || 
                     (song.artist && song.artist.toLowerCase().includes(keyword));
          });

          renderHomepageSearchResults(matches, keyword);
      });

      // B. 渲染结果 (Homepage 特别版：生成 <a> 标签)
      function renderHomepageSearchResults(songs, keyword) {
          searchDropdown.innerHTML = "";
          ...
          if (songs.length === 0) {
              ...
              songs.forEach(song => {
                  // 关键区别：这里创建一个 <a> 标签，点击直接跳转并播放
                  const item = document.createElement("a");
                  item.className = "search-item";
                  item.href = `mymusic.html?play=${song.id}`; // 跳转链接
